...
class PerspectiveCamera : public Camera {
  public:
    PerspectiveCamera(const Vector3f &center, const Vector3f &direction,
                      const Vector3f &up, float angleradians)
        : _center(center), _direction(direction.normalized()), _up(up),
          _angle(angleradians) {
    ...
    virtual Ray generateRay(const Vector2f &point) override {
        ...
        Vector3f newDir =
            d * _direction + point[0] * _horizontal + point[1] * _up;
    ...
    virtual float getTMin() const override { return 0.0f; }
