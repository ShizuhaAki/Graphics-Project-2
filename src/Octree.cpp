...
void Octree::buildNode(OctNode *parent, const Box &pbox,
                       const std::vector<int> &trigs, const Mesh &m,
                       int level) {
...
bool Octree::proc_subtree(float tx0, float ty0, float tz0, float tx1, float ty1,
                          float tz1, OctNode *node, const Ray &ray) {
    ...
    do {
        switch (currNode) {
        case 0: {
            bool result = proc_subtree(tx0, ty0, tz0, txm, tym, tzm,
                                       node->child[aa], ray);
            intersected |= result;
            currNode = new_node(txm, 4, tym, 2, tzm, 1);
        } break;
        case 1: {
            bool result = proc_subtree(tx0, ty0, tzm, txm, tym, tz1,
                                       node->child[1 ^ aa], ray);
            intersected |= result;
            currNode = new_node(txm, 5, tym, 3, tz1, 8);
        } break;
        case 2: {
            bool result = proc_subtree(tx0, tym, tz0, txm, ty1, tzm,
                                       node->child[2 ^ aa], ray);
            intersected |= result;
            currNode = new_node(txm, 6, ty1, 8, tzm, 3);
        } break;
        case 3: {
            bool result = proc_subtree(tx0, tym, tzm, txm, ty1, tz1,
                                       node->child[3 ^ aa], ray);
            intersected |= result;
            currNode = new_node(txm, 7, ty1, 8, tz1, 8);
        } break;
        case 4: {
            bool result = proc_subtree(txm, ty0, tz0, tx1, tym, tzm,
                                       node->child[4 ^ aa], ray);
            intersected |= result;
            currNode = new_node(tx1, 8, tym, 6, tzm, 5);
        } break;
        case 5: {
            bool result = proc_subtree(txm, ty0, tzm, tx1, tym, tz1,
                                       node->child[5 ^ aa], ray);
            intersected |= result;
            currNode = new_node(tx1, 8, tym, 7, tz1, 8);
        } break;
        case 6: {
            bool result = proc_subtree(txm, tym, tz0, tx1, ty1, tzm,
                                       node->child[6 ^ aa], ray);
            intersected |= result;
            currNode = new_node(tx1, 8, ty1, 8, tzm, 7);
        } break;
        case 7: {
            bool result = proc_subtree(txm, tym, tzm, tx1, ty1, tz1,
                                       node->child[7 ^ aa], ray);
            intersected |= result;
            currNode = 8;
        } break;
...
bool Octree::intersect(const Ray &ray) {
    ...
    if (std::max(std::max(tx0, ty0), tz0) <=
        std::min(std::min(tx1, ty1), tz1)) {
